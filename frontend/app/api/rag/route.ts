...
async function ensureModelExists(modelName: string) {
  try {
    ...
    const modelExists = models.models.some((model: { name: string }) => model.name === modelName);
...
// PDF 전용 검색 도구
const pdfSearchTool = tool(async (input: { query: string; limit?: number; filename?: string }) => {
  try {
    ...
    const formattedResults = results.documents?.[0]?.map((doc, index) => ({
      ...
      score: results.distances?.[0]?.[index] ? (1 - results.distances[0][index]).toFixed(4) : 'N/A'
    })) || [];
    ...
    return `PDF 검색 중 오류가 발생했습니다: ${error instanceof Error ? error.message : '알 수 없는 오류'}`;
  }
}, {
  name: 'pdf_search',
  description: 'PDF 문서에서 특정 내용을 검색합니다. PDF 파일명을 지정하면 해당 파일에서만 검색할 수 있습니다.',
  schema: z.object({
